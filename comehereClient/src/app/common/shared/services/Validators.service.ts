...
export class ValidatorsService {
  ...
  static sqlQuery(control: AbstractControl) {
    ...
    if (control.value?.toString().includes('*')) {
      return { sqlQuery: { message: 'EMAIL-EDITOR.DATA-SOURCE.ERRORS.INVALID-QUERY-ASTERICK' } };
    }
    var isValid = control.value && reg.test(control.value.toString());
    const message = {
      sqlQuery: {
        message: 'EMAIL-EDITOR.DATA-SOURCE.ERRORS.INVALID-QUERY',
      },
    };
    return isValid ? null : message;
  }
  ...
  static percentageValidation(control: AbstractControl) {
    ...
    control.setErrors({ percentageValidation: true });
    return isValid ? null : message;
  }
