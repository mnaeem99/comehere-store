...
export class GenericApiService<T> {
  ...
  public create(item: T, headers?): Observable<T> {
    return this.http.post<T>(this.url, item, { headers }).pipe(catchError(this.handleError));
  }
  ...
  public update(item: T, id: any, headers?): Observable<T> {
    return this.http.put<T>(this.url + '/' + id, item, { headers }).pipe(catchError(this.handleError));
  }
